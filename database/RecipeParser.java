...
public class parseInstr {
  ...
  public static void main(String[] args) {
    ...
    for (File file : filesInDir) {
      ...
      File outputFile = new File("./Instructions_output/"+fileJson);
  ...
  private static int getMinutesString(String[] instrAsList) {
    ...
    System.out.println(noOfMins);
    int negIndex = (noOfMins.indexOf("-"));
    if(negIndex != -1){
      noOfMins.substring(noOfMins.lastIndexOf(':') + 1);
      System.out.println(noOfMins);
    }
    try{
      int tmp = Integer.valueOf(noOfMins);
      return tmp;
    }
    catch(Exception e){
